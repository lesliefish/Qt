...
void WarningDlg::ShowWarning(const QString& title, const QString text)
{
    WarningDlg* w = new WarningDlg();
    w->ui->m_titleTextBtn->setText(title);
    w->ui->m_infoLabel->setText(text);
    w->ui->m_cancleBtn->setVisible(false);
    w->ui->m_OKBtn->setVisible(false);
    w->resize(210,180);
    w->exec();
}
